...
def outputStats():
	...
	while(True):
		...
		if(currentMinute-1 in requestStats.keys()):
			...
			a =  "Bot Detector : " + str(value) + " Minute : " + str(currentMinute -1 ) + " Request Count : " + str(total) + " Success Count : " +  str(requests) + " Dropped : " + str(dropped) + " : " + str(ipCurrentStatus)
...
class S(BaseHTTPRequestHandler):
	...
	response = ''
	if(clientIp in ipCurrentStatus and ipCurrentStatus[clientIp] == "B"):
		self._set_headers()
        	self.wfile.write("Response : " + str(response) + "\n")
		return
	...
	if(requestStats[currentMinute]['requests'] > 20):
		...
		if(clientIp in ipCurrentStatus):
